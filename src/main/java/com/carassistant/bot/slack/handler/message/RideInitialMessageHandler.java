...
public class RideInitialMessageHandler {
    ...
    @Autowired
    public RideInitialMessageHandler(@Value("${carassistant.slack.pagination_size}") int pageSize,
    ...
    public SlackApiResponse onRideList(ActionPayload payload) throws IOException, SlackApiException {
        ...
        if (ridesPage.hasContent()) {
            text = null;
            attachments = RidesView.createAttachmentsFor(localUser, ridesPage);
        } else {
    ...
    public SlackApiResponse onRideListMy(ActionPayload payload) throws IOException, SlackApiException {
        ...
        if (!rides.isEmpty()) {
            text = null;
            attachments = RidesView.createAttachmentsFor(localUser, rides);
        } else {
